    ...
    <div className="w-full min-h-screen bg-white">
     {loading && (
        <div className="w-full min-h-screen flex items-center justify-center flex-col">
          <div className="w-32 h-32 border-4 animate-spin rounded-full border-blue-500 border-t-transparent"></div>
          <div className="flex flex-row"><div className="text-black text-lg">Loading</div>
          <span className=" animate-bounce text-2xl text-black">.</span>
          <span className=" animate-bounce  text-2xl text-black">.</span>
          <span className=" animate-bounce  text-2xl text-black">.</span>
          </div>
      ...
      {!loading && (
        <div className="w-full flex items-center flex-col gap-3 relative">
          <div className="flex justify- items-center">
            <button
              className="absolute right-1 top-1 lg:text-2xl sm:text-lg md:text-xl text-blue-500 underline font-bold border border-black sm:w-[50px] sm:h[25px] md:w-[80px] lg:w-[100px] lg:h-[50px] rounded-lg"
          ...
          >
            {filteredData.length > 0 ? (
              filteredData.map((item) => (
                <div
                  key={item.name}
                  className="text-black border border-black shadow-lg rounded-md p-4"
