...
export default function Sfilter() {
  ...
  return (
    <div className="w-full min-h-screen bg-white">
      {loading && <p className="text-blue-400 text-lg text-center">...Loading</p>}
        ...
        <div className="w-full flex items-center flex-col gap-3 relative">
          <div className="flex justify-between mt-4 mx-8 fixed absolute top-1 right-1">
            <button
              className="lg:text-2xl sm:text-lg md:text-xl text-blue-500 underline font-bold border border-black sm:w-[50px] sm:h[25px] md:w-[80px] lg:h-[40px]  lg:w-[100px] lg:h-[50px] rounded-lg"
          ...
          >
            {filteredData.length > 0 ? (
              filteredData.map((item) => (
                <div
                  key={item.name}
                  className="text-black border border-black shadow-lg rounded-md p-4" onClick={() => router.push(`/11b/${item.id}`)}
                >
                  <img
                    src={item.images}
                    alt={item.name}
